...
export const prefetch = async (store: ReturnType<typeof createStore>, { seriesId }: Params) => {
  ...
  const [series, modules] = await Promise.all([
    ...
    store.getState().features.recommended.fetchRecommendedModulesByReferenceId({ referenceId: seriesId, limit: 1 }),
  ]);
      ...
      <div className="m-auto px-[24px] py-[48px]">
        <header className="mb-[24px] flex w-full flex-row items-start justify-between gap-[24px]">
          ...
          <div className="grow-1 shrink-1 overflow-hidden">
            <h1 className="mb-[16px] line-clamp-2 overflow-hidden text-ellipsis text-[32px] font-bold text-[#ffffff]">
              {series.title}
            </h1>
            <div className="line-clamp-3 overflow-hidden text-ellipsis text-[14px] text-[#999999]">
              {series.description}
            </div>
          </div>
        </header>
